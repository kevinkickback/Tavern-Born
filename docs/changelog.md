...
* The Class builder now presents required choices like other level selections inside the feature level where they are earned; Weapon Mastery offers only the weapons allowed by that class, filters them by mastery, weapon category, or attack type, and explains each mastery from the selected game data; choices stay with the class that granted them, and benefits that are no longer earned are removed after lowering a class level
...
* Added Review as the final Builder section, with direct links that reveal and highlight incomplete choices and a separate overview of calculated totals, movement, actions, content sources, and PDF readiness
