...
approved item metadata and equipment-group dependencies, closes structural class/subclass,
base-item, and source-qualified equipment references, materializes the shared 5etools item-entry
templates used by selected magic-item variants, writes stable JSON, and records per-file SHA-256
checksums. Non-SRD references embedded in marked roots are stripped only through an
explicit, reasoned audit rule. The generator rejects missing references and missing, duplicate, or
stale approvals. Before serialization it recursively removes
publication/catalog metadata (including Basic Rules flags, page/reprint links, supplemental
entries, item loot-table/search metadata, and presentation-audio references), while preserving mechanical expansion structures such
as `_versions`. It rejects image payloads and source-bearing fields outside the audited PHB/DMG/MM
and XPHB/XDMG/XMM families, removes full-book class and optional-feature catalog links from
items, and records stripped-field counts in manifest coverage. `--verify`
compares generated bytes with an existing output and rejects missing, changed, or unexpected
managed files. Manifest coverage inventories every emitted top-level entity row with its generated
path, collection, source-qualified identity, transformed-record hash, and either its single SRD
marker/version or explicit dependency approval. Generation rejects dual SRD markers, unexplained
rows, cross-file identity collisions, and approved dependencies missing from output. A snapshot
marked
`provenance-review-required` is an audit artifact and must not be added to release packaging.
`scripts/audit-srd-provenance.mjs` verifies the pinned official PDF hashes, compares normalized
user-facing passages with the matching SRD, and emits a hash-bound exception queue. Reviewed
exceptions bind the document, transformed record, and individual evidence fragment so changes
invalidate approval automatically. A release snapshot requires a clean audit; see
`docs/srd-provenance-review.md`. This source-corpus conversion and PDF-review machinery is a
one-time release-preparation tool, not maintained SRD-update infrastructure; it is removed after the
approved snapshot is materialized, while packaged-byte validation remains.
