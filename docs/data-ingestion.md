...
base-item, and source-qualified equipment references, writes stable JSON, and records per-file
SHA-256 checksums. Non-SRD references embedded in marked roots are stripped only through an
...
entries, and presentation-audio references), while preserving mechanical expansion structures such
