...
approved item metadata dependencies, closes structural class/subclass/base-item references, writes
stable JSON, and records per-file SHA-256 checksums. Non-SRD references embedded in marked roots
are stripped only through an explicit, reasoned audit rule. The generator rejects missing
references and missing, duplicate, or stale approvals. Before serialization it recursively removes
