...
describe('Spell Management - Integration Tests', () => {
  ...
  test('character creation → spell assignment → save → load cycle', () => {
    ...
    expect(
      useCharacterStore.getState().activeCharacter?.spells.spellProfiles,
    ).toHaveLength(2);
    ...
    const wizardProfile = updated?.spells.spellProfiles.find(
    ...
    const reloadedProfile = loaded?.spells.spellProfiles.find(
      (p) => p.id === 'class:Wizard|PHB',
    );
  ...
  test('spell slots structure is valid after mutations', () => {
    ...
    for (let level = 1; level <= 9; level++) {
      ...
      expect(typeof char?.spells.spellSlots[slotKey].max === 'number').toBe(
        true,
      );
      expect(typeof char?.spells.spellSlots[slotKey].used === 'number').toBe(
        true,
      );
    }
  });
