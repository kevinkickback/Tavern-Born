...
test('startup data source modal supports remote/local setup flow', async ({
  page,
}) => {
  await page.goto('/');

  await expect(
    page.getByRole('heading', { name: 'Welcome to Tavern Born' }),
  ).toBeVisible();

  await expect(page.getByRole('tab', { name: 'Remote URL' })).toBeVisible();
  await expect(
    page.getByRole('tab', { name: 'Local Directory' }),
  ).toBeVisible();
