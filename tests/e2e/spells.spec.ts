...
test('spells page shows no-character state and returns home', async ({
  page,
}) => {
...
test.describe('spells page no-character coverage', () => {
  ...
  test('main spell sections are hidden without an active character', async ({
    page,
  }) => {
    await expect(page.getByRole('button', { name: 'Add Spell' })).toHaveCount(
      0,
    );
  ...
  test('renders a single no-character card container', async ({ page }) => {
    await expect(
      page.locator('h2:has-text("No Character Selected")'),
    ).toHaveCount(1);
  });

  test('spells route stays stable on reload without a character', async ({
    page,
  }) => {
    await page.reload();
  ...
  test('no-character card remains visible after hard navigation from home', async ({
    page,
  }) => {
    await page.goto('/');
    await page.goto('/spells');
  ...
  test('does not show spell profile labels without active character', async ({
    page,
  }) => {
