...
describe('prerequisites', () => {
  test('checks class-specific level when className option is provided', () => {
    ...
    expect(
      checkPrerequisite({ level: 3 }, character, { className: 'Wizard' }),
    ).toEqual({ met: true });

    expect(
      checkPrerequisite({ level: 4 }, character, { className: 'Wizard' }),
    ).toEqual({ met: false, reason: 'Requires Wizard level 4' });
  });
  ...
  test('exports canonical prerequisite text helpers', () => {
    ...
    expect(prereqSpellToFull('Ray of Frost#c|PHB')).toBe(
      'Ray of Frost cantrip',
    );
    expect(prereqSpellToFull('Hex#x|PHB')).toBe(
