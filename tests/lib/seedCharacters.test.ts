...
describe('seedCharacters', () => {
  test('contains exactly three seed characters', () => {
    ...
    expect(DEV_SEED_CHARACTERS.map((c) => c.id)).toEqual(['seed-1', 'seed-2', 'seed-3'])
  })
  ...
  test('seed-1 is 2014 PHB human wizard with recommended-only sources and point buy', () => {
    ...
    expect(c?.portrait).toBe('/assets/images/characters/placeholder_char_card.jpg')
    expect(c?.allowedSources).toEqual(['PHB', 'XGE', 'TCE', 'MPMM', 'ERLW', 'EGW', 'MOT', 'VRGR'])
    expect(c?.spells.spellProfiles.some((p) => p.id === 'class:Wizard|PHB')).toBe(true)
    expect(
      c?.spells.spellProfiles.find((p) => p.id === 'class:Wizard|PHB')?.preparedSpells,
    ...
    const wizardLv5Selections = ['Fireball', 'Counterspell', 'Fly'].filter((spellName) => {
  ...
  test('seed-2 is 2024-only dwarf fighter with standard array', () => {
    ...
    expect(c?.portrait).toBe('/assets/images/characters/placeholder_char_card5.jpg')
    expect(c?.allowedSources).toEqual(['XPHB'])
    expect(c?.features.some((f) => f.name.includes('Fighting Style'))).toBe(true)
  ...
  test('seed-3 is all-sources tiefling with warlock/sorcerer 5/5 and custom scores', () => {
    ...
    expect(c?.backgroundSource).toBe('XPHB')
    expect(c?.details.gender).toBe('Female')
    expect(c?.portrait).toBe('/assets/images/characters/placeholder_char_card6.jpg')
    ...
    expect(c?.spells.spellProfiles.some((p) => p.id === 'class:Warlock|PHB')).toBe(true)
    expect(c?.spells.spellProfiles.some((p) => p.id === 'class:Sorcerer|PHB')).toBe(true)
    expect(
      c?.spells.spellProfiles.find((p) => p.id === 'class:Warlock|PHB')?.preparedSpells,
    ).toEqual([])
    expect(
      c?.spells.spellProfiles.find((p) => p.id === 'class:Sorcerer|PHB')?.preparedSpells,
    ).toEqual([])
    expect(c?.features.some((f) => f.name === 'Metamagic')).toBe(true)
    expect(c?.features.some((f) => f.name === 'Eldritch Invocations')).toBe(true)
  ...
  test('ability score methods are unique across three seeds', () => {
    const methods = DEV_SEED_CHARACTERS.map((c) => c.variantRules?.abilityScoreMethod)
    ...
    for (const character of DEV_SEED_CHARACTERS) {
      for (const profile of character.spells.spellProfiles) {
        ...
        for (const spellName of [...profile.cantrips, ...profile.spellsKnown]) {
          ...
          expect(classTag, `${character.id} missing attribution for ${spellName}`).toBeDefined()
        }
      }
    }
  })
