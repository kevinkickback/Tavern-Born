...
describe('characterSheetPdf', () => {
  ...
  test('should hide interactive 2014 chrome widgets while keeping portrait field', async () => {
    ...
    const outputHiddenButton = outputForm.getButton(
      'Print Button',
    ) as unknown as {
      acroField: {
    ...
    const outputPortraitButton = outputForm.getButton(
      'Portrait',
    ) as unknown as {
      acroField: {
        getWidgets: () => Array<{ getRectangle: () => { width: number } }>;
    ...
    expect(
      outputHiddenButton.acroField.getWidgets()[0].getRectangle().width,
    ).toBe(0);
    expect(outputAmmo.acroField.getWidgets()[0].getRectangle().width).toBe(0);
    expect(
      outputPortraitButton.acroField.getWidgets()[0].getRectangle().width,
    ).toBe(120);
  });
});
