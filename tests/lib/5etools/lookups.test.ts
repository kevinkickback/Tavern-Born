...
describe('5etools/lookups', () => {
  test('buildGameDataLookups creates composite-key lookups for classes, spells, features, optional features, and subclasses', () => {
    ...
    expect(
      lookups.classesByKey[getEntityLookupKey('Wizard', 'PHB')],
    ).toBeTruthy();
    expect(
      lookups.classFeaturesByKey[getEntityLookupKey('Arcane Recovery', 'PHB')],
    ).toBeTruthy();
    expect(
      lookups.spellsByKey[getEntityLookupKey('Magic Missile', 'PHB')],
    ).toBeTruthy();
    expect(
      lookups.optionalFeaturesByKey[
        getEntityLookupKey('Cantrip Formulas', 'TCE')
      ],
    ).toBeTruthy();
    expect(
      lookups.subclassesByKey[
        getSubclassLookupKey('Wizard', 'PHB', 'Evocation', 'PHB')
      ],
...
describe('buildSpellLookup', () => {
  ...
  test('cantrips (level 0) are correctly stored and distinguishable from non-cantrips', () => {
    ...
    expect(names.filter((n) => spellByName.get(n)?.level === 0)).toEqual([
      'Fire Bolt',
    ]);
    expect(names.filter((n) => spellByName.get(n)?.level !== 0)).toEqual([
      'Burning Hands',
