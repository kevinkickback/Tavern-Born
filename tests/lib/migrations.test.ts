...
describe('migrateCharacter', () => {
  ...
  it('preserves extra character fields through migration', () => {
    ...
    expect((result as Record<string, unknown>).race).toBe('Elf');
    expect((result as Record<string, unknown>).level).toBe(5);
  });
  ...
  it('throws when migration result is missing required fields (id/name)', () => {
    ...
    expect(() => migrateCharacter({}, 0)).toThrow(
      /schema validation|corrupted/i,
    );
  });
});
