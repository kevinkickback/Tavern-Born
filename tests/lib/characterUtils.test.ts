...
describe('characterUtils', () => {
  ...
  test('matchesGameDataEntry matches name and source when source exists', () => {
    expect(
      matchesGameDataEntry('Alert', 'phb', { name: 'Alert', source: 'phb' }),
    ).toBe(true);
    expect(
      matchesGameDataEntry('Alert', 'xphb', { name: 'Alert', source: 'phb' }),
    ).toBe(false);
  });
  ...
  test('countAvailableASIs uses class data levels and includes variant human bonus', () => {
    ...
    expect(
      countAvailableASIs(progression, { name: 'Variant Human' }, classesData),
    ).toBe(4);
  });
  ...
  test('calculateMaxHP computes multiclass average hit points', () => {
    ...
    expect(
      calculateMaxHP(progression, 2, { averageHp: true, classesData }),
    ).toBe(40);
  });
  ...
  test('calculateMaxHPFromScores uses constitution totals and caps effective CON at 20', () => {
    ...
    expect(
      calculateMaxHPFromScores(progression, scores, bonuses, { classesData }),
    ).toBe(15);
  });
