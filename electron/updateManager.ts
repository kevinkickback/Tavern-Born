...
const GITHUB_REPO = 'Tavern-Born'
const GITHUB_RELEASES_BASE = `https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases`
  ...
  try {
    const response = await net.fetch(url, {
      headers: {
        ...
        'User-Agent': 'tavern-born-updater',
      },
    })
  ...
  try {
    ...
    if (nextVersion && compareSemver(nextVersion, app.getVersion()) > 0) {
      const status: UpdateStatus = {
        ...
        version: nextVersion,
        changelog:
          currentStatus.status === 'available' && currentStatus.version === nextVersion
