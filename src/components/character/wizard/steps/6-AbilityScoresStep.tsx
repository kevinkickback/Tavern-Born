...
const DEFAULT_STANDARD_SCORES: Partial<Record<AbilityName, number>> =
          ...
          return Array.from({ length: block.count }, (_, slotIdx) => {
            ...
            return (
              ...
              >
                ...
                <Select
                  value={selected}
                  onValueChange={(v) => updateChoice(blockIdx, slotIdx, v)}
                >
...
function PointBuyPanel({
  ...
  const pointsUsed = ABILITY_NAMES.reduce(
    (sum, ab) => sum + (POINT_BUY_COSTS[scores[ab]] ?? 0),
    0,
  );
...
function StandardArrayPanel({
  ...
  const [assignments, setAssignments] = useState<
    Partial<Record<AbilityName, number>>
  >(() =>
    isValidStandardArrayAssignment(scores)
      ? { ...scores }
  ...
  const assign = (ability: AbilityName, raw: string) => {
    ...
    const swapTarget = ABILITY_NAMES.find(
