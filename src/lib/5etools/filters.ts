import { Race5e, Class5e, Spell5e, Background5e, Feat5e, Item5e } from '@/types/5etools'
...
export class DataFilter {
  ...
  static filterSpells(spells: Spell5e[], filters: SpellFilters): Spell5e[] {
    ...
    if (filters.classes && filters.classes.length > 0) {
      filtered = filtered.filter(s => {
        ...
        return s.classes.fromClassList.some(c => 
          filters.classes!.includes(c.name)
        )
      })
    }
    ...
    if (filters.ritual !== undefined) {
      filtered = filtered.filter(s => {
        const isRitual = (s as any).ritual === true
  ...
  static filterBackgrounds(backgrounds: Background5e[], filters: BackgroundFilters): Background5e[] {
...
export function extractUniqueSizes(races: Race5e[]): string[] {
  ...
  races.forEach(race => {
    if (race.size) {
      race.size.forEach(s => sizes.add(s))
    }
  })
...
export function extractUniqueClasses(spells: Spell5e[]): string[] {
  ...
  spells.forEach(spell => {
    if (spell.classes?.fromClassList) {
      spell.classes.fromClassList.forEach(c => classes.add(c.name))
    }
  })
