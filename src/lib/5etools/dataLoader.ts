...
export class FiveEToolsDataLoader {
      ...
      try {
        ...
        switch (resource.key) {
          ...
          case 'races':
            ...
            gameData.races.forEach(item => item.source && sourcesSet.add(item.source))
            break
          case 'backgrounds':
            gameData.backgrounds = parseBackgrounds(data) as any
            gameData.backgrounds.forEach(item => item.source && sourcesSet.add(item.source))
            break
          case 'feats':
            gameData.feats = parseFeats(data) as any
            gameData.feats.forEach(item => item.source && sourcesSet.add(item.source))
            break
          case 'items':
            gameData.items = parseItems(data) as any
            gameData.items.forEach(item => item.source && sourcesSet.add(item.source))
            break
          case 'itemsBase':
            gameData.itemsBase = parseItems(data) as any
            gameData.itemsBase.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'actions':
            gameData.actions = parseActions(data)
            gameData.actions.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'conditions':
            gameData.conditions = parseConditions(data)
            gameData.conditions.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'deities':
            gameData.deities = parseDeities(data)
            gameData.deities.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'skills':
            gameData.skills = parseSkills(data)
            gameData.skills.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'senses':
            gameData.senses = parseSenses(data)
            gameData.senses.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'languages':
            gameData.languages = parseLanguages(data)
            gameData.languages.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'magicvariants':
            gameData.magicvariants = parseMagicVariants(data)
            gameData.magicvariants.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'optionalfeatures':
            gameData.optionalfeatures = parseOptionalFeatures(data)
            gameData.optionalfeatures.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
          case 'variantrules':
            gameData.variantrules = parseVariantRules(data)
            gameData.variantrules.forEach((item: any) => item.source && sourcesSet.add(item.source))
            break
        }
      } catch (error) {
  ...
  private async loadClassData(
    ...
    for (const classFile of classFiles) {
      try {
        const classData = await this.loadResource(`class/${classFile}`, options?.signal)
        
        const parsedClasses = parseClasses(classData)
        allClasses.push(...parsedClasses)
        parsedClasses.forEach(item => item.source && sourcesSet.add(item.source))

        const parsedFeatures = parseClassFeatures(classData)
        allClassFeatures.push(...parsedFeatures)
        parsedFeatures.forEach(item => item.source && sourcesSet.add(item.source))
      } catch (error) {
  ...
  private async loadSpellData(
    ...
    for (const spellFile of spellFiles) {
      try {
        const spellData = await this.loadResource(`spells/${spellFile}`, options?.signal)
        
        const parsedSpells = parseSpells(spellData)
        allSpells.push(...parsedSpells)
        parsedSpells.forEach(item => item.source && sourcesSet.add(item.source))
      } catch (error) {
  ...
  private async loadResource(filename: string, signal?: AbortSignal): Promise<any> {
  ...
  private buildUrl(filename: string): string {
    if (this.isRemote) {
      if (filename.startsWith('class/') || filename.startsWith('books/') || filename.startsWith('spells/')) {
