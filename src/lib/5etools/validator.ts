import { DataSourceConfig } from '@/types/5etools'
import { normalizeGitHubUrl, findCorrectBranch } from './urlUtils'
import {
  RaceDataSchema,
  ClassIndexSchema,
...
const REQUIRED_FILES: FileValidationConfig[] = [
  ...
  { name: 'class/index.json', path: 'data/class/index.json', schema: ClassIndexSchema },
  { name: 'backgrounds.json', path: 'data/backgrounds.json', schema: BackgroundDataSchema },
  { name: 'spells/index.json', path: 'data/spells/index.json', schema: ClassIndexSchema },
  { name: 'feats.json', path: 'data/feats.json', schema: FeatDataSchema },
  { name: 'items.json', path: 'data/items.json', schema: ItemDataSchema },
  { name: 'items-base.json', path: 'data/items-base.json', schema: ItemDataSchema },
  { name: 'actions.json', path: 'data/actions.json', schema: ActionDataSchema },
  { name: 'conditionsdiseases.json', path: 'data/conditionsdiseases.json', schema: ConditionDataSchema },
  { name: 'deities.json', path: 'data/deities.json', schema: GenericDataSchema },
  ...
  { name: 'languages.json', path: 'data/languages.json', schema: LanguageDataSchema },
  { name: 'magicvariants.json', path: 'data/magicvariants.json', schema: GenericDataSchema },
  { name: 'optionalfeatures.json', path: 'data/optionalfeatures.json', schema: OptionalFeatureDataSchema },
  { name: 'variantrules.json', path: 'data/variantrules.json', schema: GenericDataSchema },
]
...
function buildFileUrl(basePath: string, filePath: string, isRemote: boolean): string {
  if (isRemote) {
...
export async function validateDataSource(config: DataSourceConfig): Promise<ValidationResult> {
  try {
    ...
    if (config.type === 'remote') {
      ...
      const url = new URL(config.path)
      if (url.hostname.includes('github.com') && !url.pathname.includes('/tree/') && !url.pathname.includes('/blob/')) {
    ...
    const validationPromises = REQUIRED_FILES.map(async (file) => {
      const url = buildFileUrl(normalizedPath, file.path, config.type === 'remote')
      const isValid = await validateFileStructure(url, file)
      if (isValid) {
        foundResources.push(file.name)
      }
