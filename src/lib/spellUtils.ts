// Spell display utilities — pure functions, no React/Zustand deps.
// Handles school name resolution and formatting of 5etools spell stat fields.

import type { CastingTime, SpellRange, SpellComponents, SpellDuration } from '@/types/5etools'

// ─── School names ─────────────────────────────────────────────────────────────
// 5etools uses single-letter school codes: A C D E I N T V
...
export function formatCastingTime(time: CastingTime[] | undefined): string {
  ...
  const unitDisplay = unitKey === 'bonus' ? 'Bonus Action' : unitKey.charAt(0).toUpperCase() + unitKey.slice(1)
...
// Canonical 5etools range types from SP_RANGE_TYPE_TO_FULL / SP_DIST_TYPE_TO_FULL
...
const AREA_RANGE_TYPES = new Set(['line', 'cube', 'cone', 'emanation', 'radius', 'sphere', 'hemisphere', 'cylinder'])
