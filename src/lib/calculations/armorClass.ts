// Armour class calculation — pure, no React dependencies.
// Ported from fizbanes-forge/src/services/EquipmentService.js computeArmorClass().
//
// 5etools item type codes used here:
//  'LA' = light armour   → base AC + full DEX mod
//  'MA' = medium armour  → base AC + min(DEX mod, 2)
//  'HA' = heavy armour   → base AC only (no DEX)
//  'S'  = shield         → +2 AC bonus (stacks with any armour)
//  All others            → treated as unarmed for AC purposes
...
/** True when the item is worn armour or a shield. */
