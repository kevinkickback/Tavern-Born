import { getKnownSpellNames, getProfileKnownNames } from '@/lib/calculations/spellProfiles'
...
export function buildSpellModalConfig(params: {
  ...
  const lockedNames = new Set(
    [...getKnownSpellNames(spellProfiles)].filter((name) => !ownedNames.has(name)),
  )
  ...
  const canSelectLeveledSpells = effectiveSpellLimit === null || effectiveSpellLimit > 0
