...
export function applyClassGrants(
  cls: {
    ...
    startingProficiencies?: {
      ...
      weapons?: string[]
      tools?: string[]
      toolProficiencies?: Array<Record<string, boolean | number | { choose?: { from?: string[]; count?: number } }>>
  ...
  for (const block of profs.toolProficiencies ?? []) {
    ...
    const choose = (block as any).choose as { from?: string[]; count?: number } | undefined
    if (choose) {
      const normalizedPool = (choose.from ?? []).map((entry) => normalizeGenericToolChoice(entry) ?? entry)
      const choiceRecord: ChoiceRecord = {
  ...
  // Skill choice — class typically grants "choose N from list"
