...
function canonicalizeToolName(value: string): string {
  const key = normalizeKey(value);
  if (key === "theives' tools" || key === 'theives tools')
...
export function useAvailableProficiencies(): AvailableProficiencies {
  ...
  return useMemo(() => {
    ...
    const isAllowedBySource = (
      entry: { source?: string } | null | undefined,
    ): boolean => {
    ...
    const classes = (gameData.classes ?? []).filter((cls) =>
      isAllowedBySource(cls),
    );
    const backgrounds = (gameData.backgrounds ?? []).filter((bg) =>
      isAllowedBySource(bg),
    );
    const races = (gameData.races ?? []).filter((race) =>
      isAllowedBySource(race),
    );
    ...
    for (const race of races) {
      ...
      for (const sr of (race.subraces ?? []).filter((subrace) =>
        ...
        collectFromProfBlocks(
          (sr as { toolProficiencies?: Record<string, unknown>[] })
            .toolProficiencies,
          toolMap,
        );
      }
    ...
    const isStandardLanguage = (name: string) =>
      standardSet.has(normalizeKey(name));
