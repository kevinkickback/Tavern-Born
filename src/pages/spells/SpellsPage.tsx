    ...
    () =>
      (spells as Spell5e[]).filter((s) => {
        if (
          ...
          return false;
        if (levelFilter !== 'all' && String(s.level) !== levelFilter)
          return false;
        if (classLower) {
          ...
          if (
            fromList.length > 0 &&
            !fromList.some(
              (c: { name?: string }) => c.name?.toLowerCase() === classLower,
            )
          )
            return false;
        }
                        ...
                        <div className="flex gap-1 flex-1">
                          {Array.from({ length: slot.max }, (_, i) => (
                            <button
                              ...
                              onClick={() =>
                                i < slot.used
                                  ? restoreSlot(slot.level)
                                  : consumeSlot(slot.level)
                              }
                              className={cn(
                              ...
                              title={
                                i < slot.used ? 'Restore slot' : 'Use slot'
                              }
                            />
                          ))}
                        </div>
                ...
                <Select value={levelFilter} onValueChange={setLevelFilter}>
                  ...
                  <SelectContent>
                    ...
                    {Array.from({ length: 9 }, (_, i) => (
                      <SelectItem key={`level-${i + 1}`} value={String(i + 1)}>
                        Level {i + 1}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-1.5 max-h-[550px] overflow-y-auto pr-1">
                ...
                {filteredSpells.length > 200 && (
                  <p className="text-xs text-muted-foreground text-center py-2">
                    Showing first 200 of {filteredSpells.length} — refine your
                    search
                  </p>
                )}
              </div>
...
function SpellList({
  ...
  if (!names.length) {
    return (
      <p className="text-xs text-muted-foreground py-2 text-center">None</p>
    );
  }
  return (
