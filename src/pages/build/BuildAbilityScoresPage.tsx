...
const DEFAULT_STANDARD_ARRAY_ASSIGNMENT: Partial<Record<AbilityName, number>> =
...
export function BuildAbilityScoresPage() {
  ...
  const [selectedAbility] = useState<AbilityName>('charisma');
  ...
  const skillDetailsMap = useMemo(() => {
    const map: Record<
      string,
      { name: string; entries: unknown[]; source?: string; page?: number }
    > = {};
    const rawSkills = gameData?.skills;
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <Select
                                      ...
                                      onValueChange={(v) => {
                                        const blockSels = [
                                          ...(raceAsiChoices[blockIdx] ?? []),
                                        ];
                                        const conflictIdx = blockSels.findIndex(
                                          (s, si) => si !== slotIdx && s === v,
                                        );
                                        if (conflictIdx >= 0)
                                          blockSels[conflictIdx] =
                                            blockSels[slotIdx] ?? '';
                                        blockSels[slotIdx] = v;
                                        const newChoices = [...raceAsiChoices];
                                        while (newChoices.length <= blockIdx)
                                          newChoices.push([]);
                                        newChoices[blockIdx] = blockSels;
                                        updateCharacter(character.id, {
                                          raceAsiChoices: newChoices,
                                        });
                                      }}
                                    >
