...
export function BuildProficienciesPage() {
  ...
  const toolSubtypeOptionsByKind = useMemo(() => {
    ...
    const filterBySource = (items: unknown[]) =>
      items.filter((item) => {
        const typedItem = item as { source?: string };
        if (!hasSourceFilter) return true;
        if (!typedItem?.source) return true;
        return allowedSources.includes(typedItem.source);
      });

    const collectByType = (items: unknown[], typePrefix: string): string[] => {
  ...
  const toolChoiceSlots = useMemo(() => {
    const selectedToolNorms = new Set(
      (character?.proficiencies.tools ?? []).map((name) => normalizeKey(name)),
    );
    ...
    for (const choice of ledger.choices) {
      ...
      const remaining = Math.max(
        0,
        choice.chooseCount - choice.selected.length,
      );
                  ...
                  <div className="p-4 pr-8">
                    <Tabs defaultValue="skills">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {skills.map((skill) => {
                            ...
                            const hasLedgerGrant =
                              (ledger.proficiencies.skills[normName] ?? [])
                                .length > 0;
                            const isSelected =
                              skill.proficient || hasLedgerGrant;
                            const isFixed = (
                              ledger.proficiencies.skills[normName] ?? []
                            ).some((t) => t.grantType === 'fixed');
                            const isChoiceSelected = ledger.choices.some(
                              (c) =>
                                c.domain === 'skills' &&
                                c.selected.some(
                                  (s) => normalizeKey(s) === normName,
                                ),
                            );
                            const canSelect =
                              !isSelected &&
                              ledger.choices.some(
                                (c) =>
                                  ...
                                  (c.optionPool.length === 0 ||
                                    c.optionPool.some(
                                      (p) => normalizeKey(p) === normName,
                                    )),
                              );
                            const canDeselect = isChoiceSelected && !isFixed;
                            const isFocused =
                              focused?.type === 'skill' &&
                              focused.name === skill.name;
                            return (
                              <button
                                key={skill.name}
                                type="button"
                      ...
                      <TabsContent value="saving-throws">
                        <div className="flex flex-wrap gap-2">
                          {savingThrows.map((st) => {
                            ...
                            const hasLedgerGrant =
                              (
                                ledger.proficiencies.savingThrows[
                                  normAbility
                                ] ?? []
                              ).length > 0 ||
                              (abbr
                                ? (
                                    ledger.proficiencies.savingThrows[abbr] ??
                                    []
                                  ).length > 0
                                : false);
                      ...
                      <TabsContent value="weapons">
                        <div className="flex flex-wrap gap-2">
                          {availableProficiencies.weapons
                            .filter(
                              ...
                              {toolChoiceSlots.map((slot) => (
                                ...
                                >
                                  <p className="text-xs text-muted-foreground mb-2">
                                    {slot.sourceName}: choose{' '}
                                    {formatProfLabel(slot.label)}
                                  </p>
                                  <Select
                                    onValueChange={(value) => {
                                      resolveChoiceSelection(
                                  ...
                                  >
                                    <SelectTrigger className="h-9 border-dashed">
                                      <SelectValue
                                        placeholder={`${formatProfLabel(slot.label)} (choose type)`}
                                      />
                                    </SelectTrigger>
                                    <SelectContent>
                          ...
                          {availableProficiencies.tools.length > 0 ? (
                            visibleToolPills
                              ...
                              .map((toolName) => {
                                ...
                                const isFixed = (
                                  ledger.proficiencies.tools[normTool] ?? []
                                ).some((t) => t.grantType === 'fixed');
                                const isChoiceSelected = ledger.choices.some(
                                  (c) =>
                                    c.domain === 'tools' &&
                                    c.selected.some(
                                      (s) => normalizeKey(s) === normTool,
                                    ),
                                );
                                const canSelect =
                                  !isSelected &&
                                  ledger.choices.some(
                                    (c) =>
                                      ...
                                      (c.optionPool.length === 0 ||
                                        c.optionPool.some(
                                          (p) => normalizeKey(p) === normTool,
                                        )),
                                  );
                                const canDeselect =
                                  isChoiceSelected && !isFixed;
                                return (
                                  <button
                                    key={toolName}
                                    type="button"
                            ...
                            <p className="text-muted-foreground text-sm">
                              No tools available in game data
                            </p>
                          )}
                        </div>
                      </TabsContent>
                      ...
                      <TabsContent value="languages">
                        <div className="flex flex-wrap gap-2">
                          {availableProficiencies.languages.length > 0 ? (
                            availableProficiencies.languages
                              .filter(
                              ...
                              .map((langName) => {
                                ...
                                const hasLedgerGrant =
                                  (
                                    ledger.proficiencies.languages[normLang] ??
                                    []
                                  ).length > 0;
                                const isSelected =
                                  hasProfInArray(
                            ...
                            <p className="text-muted-foreground text-sm">
                              No languages available in game data
                            </p>
                          )}
                        </div>
                      </TabsContent>
                  ...
                  <div className="p-4">
                    {!focused ? (
                      ...
                      <div className="space-y-3">
                        ...
                        {skillDescriptions[focused.name.toLowerCase()]?.length >
                          0 && (
                          <div>
                            <h4 className="text-xs font-bold text-accent uppercase tracking-wider mb-2">
                              Description
                            </h4>
                            {skillDescriptions[focused.name.toLowerCase()].map(
                              (e) => (
                                <div
                                  key={
                                    typeof e === 'string'
                        ...
                        <div className="space-y-2">
                          ...
                          {!hasProfInArray(
                            character.proficiencies[
                              focused.category.toLowerCase() as
                                | 'armor'
                                | 'weapons'
                                | 'tools'
                                | 'languages'
