...
interface BuildProficienciesTabsPanelProps {
  ...
  ledger: {
    choices: Array<{
      ...
      selected: string[];
      optionPool: string[];
      sourceTag: { sourceName: string };
    }>;
    proficiencies: {
      skills: Record<string, Array<{ grantType: string }>>;
      savingThrows: Record<string, Array<{ grantType: string }>>;
      tools: Record<string, Array<{ grantType: string }>>;
      languages: Record<string, Array<{ grantType: string }>>;
    };
  };
  choiceCounts: Record<
      ...
      <TabsContent value="skills">
        <div className="flex flex-wrap gap-2">
          {skills.map((skill) => {
            ...
            const isChoiceSelected = ledger.choices.some(
              (choice) =>
                ...
                choice.selected.some(
                  (selected) => normalizeKey(selected) === normName,
                ),
            );
            const canSelect =
              !isSelected &&
            ...
            const isFocused =
              focused?.type === 'skill' && focused.name === skill.name;
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (canDeselect)
                    onResolveChoiceSelection('skills', skill.name, false);
                  else if (canSelect)
      ...
      <TabsContent value="saving-throws">
        <div className="flex flex-wrap gap-2">
          {savingThrows.map((save) => {
            ...
            const hasLedgerGrant =
              (ledger.proficiencies.savingThrows[normAbility] ?? []).length >
                0 ||
              (abbr
                ? (ledger.proficiencies.savingThrows[abbr] ?? []).length > 0
                : false);
            const isSelected = save.proficient || hasLedgerGrant;
            const isFocused =
              focused?.type === 'save' && focused.ability === save.ability;
            ...
            availableLanguages.map((languageName) => {
              ...
              const isChoiceSelected = ledger.choices.some(
                (choice) =>
                  ...
                  choice.selected.some(
                    (selected) => normalizeKey(selected) === normLang,
                  ),
              );
              const canSelect =
                !isSelected &&
                ledger.choices.some(
                  (choice) =>
                    ...
                    ((choice.optionPool.length > 0 &&
                      ...
                      (choice.optionPool.length === 0 &&
                        isStandardLanguage(languageName))),
                );
              const canDeselect = isChoiceSelected && !isFixed;
